...
import splitFragments, { joinFragments } from './splitFragments.mjs';
 ...
 */
export default function parseParts(element, options, tagSettings) {
	const events = {};
	const constants = {};
	const parts = new Map();
	...
	for (let i = 0; i < element.attributes.length; i++) {
		...
		const fragments = splitFragments(value, tagSettings);
	...
	if (!(element instanceof HTMLTemplateElement)) {
		...
		for (let i = 0, node = element.firstChild; node !== null; i++, node = node.nextSibling) {
			...
			const fragments = isTextNode ? splitFragments(text, tagSettings) : null;
...
export function contextSourceCode(parseResult, options) {
	const context = {};
	const tags = config(options, 'tagsName');
